...
internal class ServiceTypeSelector : IServiceTypeSelector, ISelector
{
    ...
    {
        if (Selectors.Count == 0) AsSelf();

        var strategy = RegistrationStrategy ?? registrationStrategy;

        foreach (var selector in Selectors) selector.Populate(services, strategy);
    }
    ...
    {
        foreach (var selector in Selectors.OfType<LifetimeSelector>()) selector.Lifetime = lifetime;
    }
