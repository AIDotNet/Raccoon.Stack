...
internal static class ReflectionExtensions
{
    public static bool IsNonAbstractClass(this Type type, bool publicOnly)
    {
        ...
        {
            if (type.HasAttribute<CompilerGeneratedAttribute>()) return false;

            if (publicOnly) return type.IsPublic || type.IsNestedPublic;
    ...
    {
        if (otherType.IsGenericTypeDefinition) return type.IsAssignableToGenericTypeDefinition(otherType);
    ...
    public static IEnumerable<Type> FindMatchingInterface(this Type type,
        Action<Type, IImplementationTypeFilter>? action)
    {
        ...
        if (matchedInterfaces.Length == 0) yield break;
        ...
        if (matchingType is null) yield break;
    ...
    {
        if (!type.IsGenericType) return type.GetInterfaces();

        if (!type.IsGenericTypeDefinition) return type.GetInterfaces();
