...
{
    [Obsolete("This method has been marked obsolete and will be removed in the next major version. Use " +
              nameof(FromTypes) + " instead.")]
    public static IServiceTypeSelector AddType<T>(this ITypeSelector selector)
    {
        return selector.FromType<T>();
    }
    ...
    {
        return selector.FromTypes<T1, T2>();
    }
    ...
    {
        return selector.FromTypes<T1, T2, T3>();
    }
