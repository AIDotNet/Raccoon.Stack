using Raccoon.Stack.Core.AutoRaccoon;
...
public static partial class ServiceCollectionExtensions
{
    ...
    /// <exception cref="DecorationException">
    ...
    public static IServiceCollection Decorate<TService>(this IServiceCollection services,
        Func<TService, TService> decorator) where TService : notnull
    {
        Preconditions.NotNull(services, nameof(services));
    ...
    /// </exception>
    public static IServiceCollection Decorate<TService>(this IServiceCollection services,
        Func<TService, IServiceProvider, TService> decorator) where TService : notnull
    {
        Preconditions.NotNull(services, nameof(services));
        Preconditions.NotNull(decorator, nameof(decorator));
    ...
    {
        if (services.TryDecorate(strategy)) return services;
    ...
    public static bool TryDecorate(this IServiceCollection services, DecorationStrategy strategy)
    {
        ...
        for (var i = services.Count - 1; i >= 0; i--)
        {
            ...
            if (serviceDescriptor.ServiceType is DecoratedType) continue; // Service has already been decorated.

            if (!strategy.CanDecorate(serviceDescriptor
                    .ServiceType)) continue; // Unable to decorate using the specified strategy.
