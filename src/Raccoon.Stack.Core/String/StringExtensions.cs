...
public static class StringExtensions
{
    ...
    public static bool IsIdentityCardId(this string value)
    {
        ...
        if (!regex.Match(value).Success) return false;
        array = regex.Split(value);
        if (!DateTime.TryParse($"{array[2]}-{array[3]}-{array[4]}", out time)) return false;
        //校验最后一位
    ...
    public static string ToSingular(this string word)
    {
        ...
        if (plural1.IsMatch(word)) return plural1.Replace(word, "${keep}y");
        if (plural2.IsMatch(word)) return plural2.Replace(word, "${keep}");
        if (plural3.IsMatch(word)) return plural3.Replace(word, "${keep}");
        if (plural4.IsMatch(word)) return plural4.Replace(word, "${keep}");
    ...
    public static string ToPlural(this string word)
    {
        ...
        if (plural1.IsMatch(word)) return plural1.Replace(word, "${keep}ies");
        if (plural2.IsMatch(word)) return plural2.Replace(word, "${keep}s");
        if (plural3.IsMatch(word)) return plural3.Replace(word, "${keep}es");
        if (plural4.IsMatch(word)) return plural4.Replace(word, "${keep}s");
    ...
    {
        return value.TrimEnd(trimParameter, StringComparison.CurrentCulture);
    }
