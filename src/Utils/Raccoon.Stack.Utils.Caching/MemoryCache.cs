...
namespace Raccoon.Stack.Utils.Caching;

public class MemoryCache<TKey, TValue> : IDisposable where TKey : notnull
{
    private readonly ConcurrentDictionary<TKey, Lazy<TValue>> _dicCache = new();

    public TKey[] Keys => _dicCache.Keys.ToArray();

    public TValue[] Values
    {
        get { return _dicCache.Values.Select(value => value.Value).ToArray(); }
    }
    ...
    public TValue GetOrAdd(TKey key, Func<TKey, TValue> valueFactory)
    {
        ...
        {
            lazyValue = _dicCache.GetOrAdd(key,
